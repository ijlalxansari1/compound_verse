...
function analyzeDomains(entries: Entry[]): { dominant: string | null; weakest: string | null } {
    ...
    const totals = {
        health: entries.reduce((sum, e) => sum + (e.domains?.['health'] ?? 0), 0),
        faith: entries.reduce((sum, e) => sum + (e.domains?.['faith'] ?? 0), 0),
        career: entries.reduce((sum, e) => sum + (e.domains?.['career'] ?? 0), 0)
    };
