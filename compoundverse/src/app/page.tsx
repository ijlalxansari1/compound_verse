...
export default function Home() {
  ...
  // One-time Hydration Effect for New Users (Premium Onboarding)
  useEffect(() => {
    const pending = localStorage.getItem('pending_habits');

    // Only proceed if we have pending habits and auth is settled
    if (pending && !isAuthLoading && !isHabitsLoading && user) {
      // Safety check: if user already has initialized domains in state, we might skip?
      // But since state updates are async, activeDomains might be empty initially.
      // Reliance on 'pending_habits' existence is the primary guard.
      ...
      try {
        ...
        import('@/lib/domains').then(({ getDomains, updateDomainItems, addDomain }) => {
          ...
          // Force window reload to refresh domains
  ...
  }, [user, isAuthLoading, isHabitsLoading]);
